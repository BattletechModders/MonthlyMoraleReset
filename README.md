...
* The reset is applied before the expenditure level is selected, ensuring the selected bonus or malus affect the next period correctly.
* The reset base value is the StartingMorale value from SimGameConstants.json. This is overridden by the mod to allow fine tuning to your linking. The basic value is still the default 25.
* The reset base value is adjusted by the Argo active (already built) upgrades.
* The mod also overrides the expenditure levels morale maluses and bonuses. The basic modded values are -10, -5, +5 and +10 from the
default -4, -2, +1 and +2.
## Description
